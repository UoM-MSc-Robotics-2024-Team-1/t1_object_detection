...
#     parser.add_argument("--path", type=str, default=r'C:\Users\25166\Desktop\dataset\cube_img', help="images save path")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        aligned_frames = align.process(frames)
        try:
            rgb = get_aligned_images(dirname, aligned_frames, depth_scale)
            cv2.imshow('RGB image', rgb)
            key = cv2.waitKey(1)
            if key == ord('q') or key == ord('Q'):
                pipeline.stop()
